...
- (id) init
{
   if ( (self = [super init]) ) {
      ...
      circleClosureDistanceVariance_ = 200.0;
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
   ...
   if ( distanceBetweenPoints(firstTouch_, endPoint) > circleClosureDistanceVariance_ ) {
      [self failWithError:CircleGestureErrorNotClosed];
      return;
   }
   
   // Took too long to draw
   if ( [NSDate timeIntervalSinceReferenceDate] - firstTouchTime_ > maximumCircleTime_ ) {
      [self failWithError:CircleGestureErrorTooSlow];
      return;
   }
   
   // Not enough points
   if ( [points_ count] < minimumNumPoints_ ) {
      [self failWithError:CircleGestureErrorTooShort];
      return;
   }
   ...
   for ( NSString *onePointString in points_ ) {
      ...
      if ( (pointAngle > currentAngle && hasSwitched) && (index < [points_ count] - overlapTolerance_) ) {
         [self failWithError:CircleGestureErrorOverlapTolerance];
         return;
      }
